...
def voicenet(input_x, reuse=False, is_training=True):
    with tf.name_scope('audio_embedding_network'):
        ...
        with tf.compat.v1.variable_scope('conv1') as scope:
            conv1_1 = tf.keras.layers.Conv2D(input_x, filters=96, kernel_size=[7,7], strides=[2,2], padding='SAME', reuse=reuse, name='cc1')
            conv1_1 = tf.keras.layers.batch_normalization(conv1_1, training=is_training, name='bbn1', reuse=reuse)
            conv1_1 = tf.nn.relu(conv1_1)
            conv1_1 = tf.keras.layers.max_pooling2d(conv1_1, pool_size=[3,3], strides=[2,2], name='mpool1')
            
        with tf.compat.v1.variable_scope('conv2') as scope:
            conv2_1 = tf.keras.layers.Conv2D(conv1_1, filters=256, kernel_size=[5,5], strides=[2,2], padding='SAME', reuse=reuse, name='cc2')
            conv2_1 = tf.keras.layers.batch_normalization(conv2_1, training=is_training, name='bbn2', reuse=reuse)
            conv2_1 = tf.nn.relu(conv2_1)
            conv2_1 = tf.keras.layers.max_pooling2d(conv2_1, pool_size=[3,3], strides=[2,2], name='mpool2')


        with tf.compat.v1.variable_scope('conv3') as scope:
            conv3_1 = tf.keras.layers.Conv2D(conv2_1, filters=384, kernel_size=[3,3], strides=[1,1], padding='SAME', reuse=reuse, name='cc3_1')
            conv3_1 = tf.keras.layers.batch_normalization(conv3_1, training=is_training, name='bbn3', reuse=reuse)
            conv3_1 = tf.nn.relu(conv3_1)
            
            conv3_2 = tf.keras.layers.Conv2D(conv3_1, filters=256, kernel_size=[3,3], strides=[1,1], padding='SAME', reuse=reuse, name='cc3_2')
            conv3_2 = tf.keras.layers.batch_normalization(conv3_2, training=is_training, name='bbn4', reuse=reuse)
            conv3_2 = tf.nn.relu(conv3_2)

            conv3_3 = tf.keras.layers.Conv2D(conv3_2, filters=256, kernel_size=[3,3], strides=[1,1], padding='SAME', reuse=reuse, name='cc3_3')
            conv3_3 = tf.keras.layers.batch_normalization(conv3_3, training=is_training, name='bbn5', reuse=reuse)
            conv3_3 = tf.nn.relu(conv3_3)
            conv3_3 = tf.keras.layers.max_pooling2d(conv3_3, pool_size=[5,3], strides=[3,2], name='mpool3')

        with tf.compat.v1.variable_scope('conv4') as scope:
            conv4_3 = tf.keras.layers.Conv2D(conv3_3, filters=4096, kernel_size=[9,1], strides=[1,1], padding='VALID', reuse=reuse, name='cc4_1')
            conv4_3 = tf.keras.layers.batch_normalization(conv4_3, training=is_training, name='bbn6', reuse=reuse)
            conv4_3 = tf.nn.relu(conv4_3)
            # conv4_3 = tf.keras.layers.average_pooling2d(conv4_3, pool_size=[1, conv4_3.shape[2]], strides=[1,1], name='apool4')
    ...
    features = tf.keras.layers.dense(flattened, 1024, reuse=reuse, name='fc_audio_vgg')
...
def train(opt):
    ...
    x = tf.compat.v1.placeholder(tf.float32, [None, 512, 300, 1], name='audio_input')
    ...
    emb = tf.keras.layers.dense(emb_ori, n_classes)
...
def veri(opt, reuse=True):
    ...
    # x_test = tf.compat.v1.placeholder(tf.float32, [None, 512, None, 1], name='audio_test')
    # y_test = tf.compat.v1.placeholder(tf.int64, [None])
    ...
    test_322 = tf.compat.v1.placeholder(tf.float32, [None, n_classes])
    ...
    emb_test = tf.compat.v1.placeholder(tf.float32, [None, n_classes])

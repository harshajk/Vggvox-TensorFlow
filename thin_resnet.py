...
def identity_block2d(input_tensor, kernel_size, filters, stage, block, is_training, reuse, kernel_initializer=tf.contrib.layers.xavier_initializer(uniform=False)):
	...
	x = tf.keras.layers.Conv2D(input_tensor, filters1, (1, 1), use_bias=False, kernel_initializer=kernel_initializer, name=conv_name_1, reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name=bn_name_1, reuse=reuse)
	x = tf.nn.relu(x)

	conv_name_2 = 'conv' + str(stage) + '_' + str(block) + '_3x3'
	bn_name_2   = 'bn'   + str(stage) + '_' + str(block) + '_3x3'
	x = tf.keras.layers.Conv2D(x, filters2, kernel_size, use_bias=False, padding='SAME', kernel_initializer=kernel_initializer, name=conv_name_2, reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name=bn_name_2, reuse=reuse)
	x = tf.nn.relu(x)

	conv_name_3 = 'conv' + str(stage) + '_' + str(block) + '_1x1_increase'
	bn_name_3   = 'bn'   + str(stage) + '_' + str(block) + '_1x1_increase'
	x = tf.keras.layers.Conv2D(x, filters3, (1,1),  use_bias=False, kernel_initializer=kernel_initializer, name=conv_name_3, reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name=bn_name_3, reuse=reuse)
...
def conv_block_2d(input_tensor, kernel_size, filters, stage, block, is_training, reuse, strides=(2, 2), kernel_initializer=tf.contrib.layers.xavier_initializer(uniform=False)):
	...
	conv_name_1 = 'conv' + str(stage) + '_' + str(block) + '_1x1_reduce'
	bn_name_1   = 'bn'   + str(stage) + '_' + str(block) + '_1x1_reduce'
	x = tf.keras.layers.Conv2D(input_tensor, filters1, (1, 1), strides=strides, use_bias=False, kernel_initializer=kernel_initializer, name=conv_name_1, reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name=bn_name_1, reuse=reuse)
	x = tf.nn.relu(x)

	conv_name_2 = 'conv' + str(stage) + '_' + str(block) + '_3x3'
	bn_name_2   = 'bn'   + str(stage) + '_' + str(block) + '_3x3'
	x = tf.keras.layers.Conv2D(x, filters2, kernel_size, padding='SAME', use_bias=False, kernel_initializer=kernel_initializer, name=conv_name_2, reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name=bn_name_2, reuse=reuse)
	x = tf.nn.relu(x)
	...
	shortcut = tf.keras.layers.Conv2D(input_tensor, filters3, (1,1), use_bias=False, strides=strides, kernel_initializer=kernel_initializer, name=conv_name_4, reuse=reuse)
	shortcut = tf.keras.layers.batch_normalization(shortcut, training=is_training, name=bn_name_4, reuse=reuse)
...
def resnet34(input_tensor, is_training=True, pooling_and_fc=True, reuse=False, kernel_initializer=tf.contrib.layers.xavier_initializer(uniform=False)):
	x = tf.keras.layers.Conv2D(input_tensor, 64, (7,7), strides=(1,1), kernel_initializer=kernel_initializer, use_bias=False, padding='SAME', name='voice_conv1_1/3x3_s1', reuse=reuse)
	x = tf.keras.layers.batch_normalization(x, training=is_training, name='voice_bn1_1/3x3_s1', reuse=reuse)
	x = tf.nn.relu(x)
	x = tf.keras.layers.max_pooling2d(x, (2,2), strides=(2,2), name='voice_mpool1')
	...
	if pooling_and_fc:
		pooling_output = tf.keras.layers.max_pooling2d(x4, (3,1), strides=(2,2), name='voice_mpool2')
		# fc_output      = tf.keras.layers.Conv2D(pooling_output, 512, (7, 1), name='fc1', reuse=reuse)
